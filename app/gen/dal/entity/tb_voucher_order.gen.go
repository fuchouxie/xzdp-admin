...
type TbVoucherOrder struct {
	...
	PayType    bool      `gorm:"column:pay_type;not null;default:1" json:"pay_type"`                       // 支付方式 1：余额支付；2：支付宝；3：微信
	Status     bool      `gorm:"column:status;not null;default:1" json:"status"`                           // 订单状态，1：未支付；2：已支付；3：已核销；4：已取消；5：退款中；6：已退款
	CreateTime time.Time `gorm:"column:create_time;not null;default:CURRENT_TIMESTAMP" json:"create_time"` // 下单时间
